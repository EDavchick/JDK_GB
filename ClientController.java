package org.less02_server.client.domain;

import org.less02_server.client.ui.ClientView;
...
public class ClientController {
     ...
     * @param name имя пользователя, которым будем подписывать
     * исходящие сообщения
    ...
    public void message(String text) {
        if (connected) {
            if (!text.isEmpty()) {
                serverController.message(STR."\{name}: \{text}");
            }
        } else {
    ...
    private void showOnWindow(String text) {
        clientView.showMessage(STR."\{text}\n");
    }
